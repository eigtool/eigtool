...
%   positive semi-definite and the function Y = AFUN(X) must return
%   Y = (A-SIGMA*B)\X, which is Y = A\X when SIGMA = 0 or SIGMA = 'SM'.
%
...
%   $Revision: 1.39 $  $Date: 2001/02/16 21:49:00 $
%
%   Modifications for use with EigTool by Tom Wright, October 2002
        ...
        if Amatrix
            if (mode == 1)
                if isempty(B)
                    ...
                    % OP = R'\(A*(R\x))
                    if issparse(B)
                        workd(permB,col2) = RBT \ (A * (RB \ workd(permB,col1)));
                    else
            ...
            if (mode == 1)
                if isempty(B)
                    ...
                    % OP = R'\(A*(R\x))
                    if issparse(B)
                        workd(permB,col2) = RBT \ ...
                            feval(A,(RB\workd(permB,col1)), ...
                            varargin{7-Amatrix-Bnotthere:end});
                    else
                        workd(:,col2) = RBT \ ...
