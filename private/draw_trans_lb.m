...
% Now compute the correct bound based on the norm of the resolvent (1/Ss)
      if strcmp(ftype,'E'),
        ...
        eat = exp(alp*t_pts);
       %bnd = eat./(1+(eat-1)/(alp*R));    %%% EIGTOOL
        aA=.5*max(eig(A+A')); bnd=eat-(eat-exp(aA*t_pts))/((alp-aA)*R); %%% REVISION
        max_bnd = max(bnd);
%        bnd = max(1,the_pt(1)/Ss);
      ...
      elseif strcmp(ftype,'P'),
        ...
        rk = r.^k;
       %bnd = rk ./ (1+(rk-1)/((r-1)*(r*R-1)));         %%% EIGTOOL 
        nA = norm(A); bnd = rk - (rk-nA.^k)./((r-nA)*R);   %%% REVISION
        max_bnd = max(bnd);
%        bnd = max(1,(abs(the_pt(1)+1i*the_pt(2))-1)/Ss);
   ...
   %h = plot(data_pts,bnd,'g-','Parent',cax1,'linewidth',2); %%% EIGTOOL
    h = plot(data_pts,bnd,'m-','Parent',cax1,'linewidth',2); %%% REVISION
...
% Append the new handle to the userdata
   ...
   %hs = semilogy(data_pts,bnd,'g-','Parent',cax2,'linewidth',2); %%% EIGTOOL
    hs = semilogy(data_pts,bnd,'m-','Parent',cax2,'linewidth',2); %%% REVISION
